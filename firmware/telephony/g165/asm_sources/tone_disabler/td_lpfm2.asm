...
;
;  Modulation, Low pass filtering and zero cross detection for SND channel.
;
;  Symbols Used :
;       sin_sample        : Input Sample from SND channel
;       zero_cross2       : Zero cross flag for SND ch.
;       sine_p2           : Pointer for sine table (freq 2000 Hz)
;       alp_states2[3]    : All-pole states for LPF
;       alp_states2_p     : Pointer for all-pole states for LPF
;       blp_states2[3]    : All-zero states for LPF
;       blp_states2_p     : Pointer for all-zero states for LPF
...
;
;  Input  :
;       sin_sample       = | s.fff ffff | ffff ffff |     in x:rin_sample
;
;  Output :
...
;       blp_states2_p    = |  iiii iiii | iiii iiii |
;       sine_2000 Table (for 2000 Hz sine wave in 2.14 format):
;          (Pointer for the sine_2000 table is stored in sine_p2 using r0)
;
;                                +<-----<-----+
;                                |            |
;                      +-------------------+  |
...
;       (Pointers are stored in blp_states2_p, alp_states2_p using r0)
;                               
;                                +<-----<-----+
;                                |            |
;                      +-------------------+  |
;              r0----->|  blp_states2(n-3) |  |
;       (incremented   |-------------------|  |
;        by 1 after    |  blp_states2(n-2) |  |
;        every call)   |-------------------|  |
;                      |  blp_states2(n-1) |  | (Modulo 3)
;                      |-------------------|  |
;                                |            |
;                                +----->----->+
;                       input states buffer       
...
;              r0----->|  alp_states2(n-3) |  |
;       (incremented   |-------------------|  |
;        by 1 after    |  alp_states2(n-2) |  |
;        every call)   |-------------------|  |
;                      |  alp_states2(n-1) |  | (Modulo 3)
;                      |-------------------|  |
;                                |            |
;                                +----->----->+
...
;          (Table entries 0,1,0,-1 in 2.14 format pointed by sine_p2)
;
;               xn2 = sin_sample*(*sine_p2++);
;
;          %%% Low Pass filtering for channel 2 %%%
;       
;          tmp = blp_coef(0)*xn2 + blp_coef(1:3)'*blp_states2(0:2)
;          tmp = tmp/1024                      % tmp in accumulator %
;           
;          blp_states2(1:2) = blp_states2(0:1)
;          blp_states2(0) = xn2
;       
;          tmp = tmp - alp_coefs(1:3)'*alp_states2(0:2)
;          tmp = tmp*4 + 2^(-16)
;           
;          yn2 = (floor(tmp*32768))/32768  % Store result in 1.15 form %
;       
;          alp_states2(1:2) = alp_states2(0:1)
;          alp_states2(0) = yn2
;       
;          %%% Zero cross detect for channel 2 %%%
;       
;          prev_yn2  = alp_states2(1)
;       
;          % Implemented using xoring of prev_yn2 and yn2 %
;
;          If ( (prev_yn2 >= 0) & (yn2 < 0) )
;               zero_cross2 = 1
;          Elseif ( (prev_yn2 < 0) & (yn2 >= 0) )
;               zero_cross2 = 1
;          Else
;               zero_cross2 = 0
;          Endif
...
; All-zero section of LPF
	...
	rep     #8                        ;tmp = tmp/256
	asr     a                                     
	move    r0,x:blp_states2_p        ;Store the state pointer

; All-pole section of LPF
	
	move    x:alp_states2_p,r0        ;Set states pointer 
					  ;r3 points to coeffs
	...
	move    r0,x:alp_states2_p        ;Store the state pointer 
...
; Zero Cross Detection
	...
	move    b,x:zero_cross2           ;Update zero_cross flag

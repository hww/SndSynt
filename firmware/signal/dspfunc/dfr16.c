                  ...
                  CFrac16 *pZ)
{
    ...
    /* The remaining loops .... */
    temp16 = (pCFFT->options) & FFT_SCALE_RESULTS_BY_N;
    ...
    /*--------------------------------------------------------------*/
    temp16 = (pCFFT->options) & FFT_OUTPUT_IS_BITREVERSED;
    if (!temp16)
    {
    ...
    if (temp16)
    {
        ...
        /* Put back the original saturation bit */
        return ((Int32) (pCFFT->No_of_Stages));  
    }
    else if ((pCFFT->options) & FFT_SCALE_RESULTS_BY_DATA_SIZE)
    {
        /* Put back the original saturation bit */
        return ((Int32) ScaleBF);  /* Return amount of scaling done */
    }
    else
    {
        /* Put back the original saturation bit */
...
Result dfr16RFFTC (dfr16_tRFFTStruct *pRFFT, Frac16 *pX, dfr16_sInplaceCRFFT *pZ)
{
    ...
    Int16 But_Top;       /* Offset for b-fly top */
    Int16 But_Bot;       /* Offset for b-fly bottom */
    Int16 Bf_Size, Passes;       /* Size of butterfly */
    ...
    if (((pRFFT->options) & FFT_SCALE_RESULTS_BY_N) || Sticky)
    {
        ...
        {
            But_Top = Gfs;
            But_Bot = pRFFT->n/2 + Gfs;
        
            temp1 = L_deposit_h(pY[But_Top].real);
            pW[But_Top].real = round(L_add(L_deposit_h(pY[But_Bot].real),temp1)/2);
            pW[But_Bot].real = round(L_sub(temp1,L_deposit_h(pW[But_Top].real)));
            temp1 = L_deposit_h(pY[But_Top].imag);
            pW[But_Top].imag = round(L_add(temp1,L_deposit_h(pY[But_Bot].imag))/2);
    ...
    /* The remaining loops .... */
    temp16 = (pRFFT->options) & FFT_SCALE_RESULTS_BY_N; /* Look for scaling options */
        ...
        for(Gfs = 0; Gfs < Passes; Gfs++)
        {
            for(Bfs = 0; Bfs < N; Bfs++)
            {
                ...
                if (temp16 || Sticky)
                {
                    ...
                    pW[But_Top].real = round(L_add(temp1,temp2)/2);                       /* WrBr+WiBi+Ar */
                    ...
                    pW[But_Top].imag = round(L_add(temp2,temp1)/2);                       
    ...
    if (temp16 || Sticky)
    {
        pW[0].real = round(L_add(L_deposit_h(A), temp2)/2);
    ...
    for(i = 1; i <= N/2; i++)
    {
        ...
        if (temp16 || Sticky)
        {
            pW[i].real = round(L_add(L_deposit_h(A),temp2)/2);
        ...
        if (temp16 || Sticky)
        {
            pW[i].imag = round(L_add(L_deposit_h(D),temp2)/2);
    ...
    temp16 = (pRFFT->options) & FFT_SCALE_RESULTS_BY_N;
    
    if (temp16)
    {
        /* Scaling factor = N. Thus, no. of shifts = No_of_Stages */
        
        return ((Int32) ((pRFFT->No_of_Stages) + 1));  
    }
                  ...
                  CFrac16 *pZ)
{
    ...
    /* The remaining loops .... */
    temp16 = (pCIFFT->options) & FFT_SCALE_RESULTS_BY_N; /* Look for scaling options */
    ...
    /* Rearrange the output array in bit-reversed order, if required */
    /*--------------------------------------------------------------*/
    temp16 = (pCIFFT->options) & FFT_OUTPUT_IS_BITREVERSED;
    if (!temp16)
    {
    ...
    /*----------------------------------------------------------------*/
    archGetSetSaturationMode (SatBit);
    temp16 = (pCIFFT->options) & FFT_SCALE_RESULTS_BY_N;
    if (temp16)
    {
        /* Scaling factor = N. Thus, no. of shifts = No_of_Stages */
        
        /* Put back the original saturation bit */
        return ((Int32) (pCIFFT->No_of_Stages));  
    }
...
Result  dfr16RIFFTC (dfr16_tRFFTStruct * pRIFFT, dfr16_sInplaceCRFFT *pX, Frac16 *pZ)
{
    ...
    /* The remaining loops .... */
    temp16 = (pRIFFT->options) & FFT_SCALE_RESULTS_BY_N; /* Look for scaling options */
        ...
        for(Gfs = 0; Gfs < Passes; Gfs++)
        {
            for(Bfs = 0; Bfs < N; Bfs++)
            {
                ...
                if (temp16 || Sticky)
                {
                    ...
                    temp1 = L_negate(L_mult(negate(pRIFFT->Twiddle_br[Gfs].imag),temp));    /*-WiBr*/        
    ...
    archGetSetSaturationMode (SatBit);    /* Set the saturation mode bit */
    temp16 = (pRIFFT->options) & FFT_SCALE_RESULTS_BY_N;
    if (temp16)
    {
        /* Scaling factor = N. Thus, no. of shifts = No_of_Stages */
        
        return ((Int32) ((pRIFFT->No_of_Stages)+1));  
    }
...
void dfr16FIRInit (dfr16_tFirStruct * pFIR, Frac16 *pC, UInt16 n)
{
	...
	pFIRPriv -> bCanUseModAddr = memIsAligned(pFIRPriv -> pHistory, n);     
...
void dfr16FIRIntInit  (dfr16_tFirIntStruct *pFIRInt, Frac16 *pC, UInt16 n, UInt16 f)
{
	...
	pFIRIntPriv -> bCanUseModAddr   = memIsAligned(pFIRIntPriv -> pHistory, pFIRIntPriv -> Count);     
...
void dfr16IIRInit (dfr16_tIirStruct * pIir, Frac16 *pC, UInt16 nbiq)
{
	...
	assert (pIir -> pHistory != NULL); 
			...
			pIir -> bCanUseModAddr = memIsAligned(pIir -> pHistory, nbiq * FILT_STATES_PER_BIQ);     
...
Result dfr16AutoCorrC(UInt16 options, Frac16 *pX, Frac16 *pZ, UInt16 nx, UInt16 nz)
	{
      ...
      for(j = nx-1; j >= 0; j--)
      {
     		switch(options)
         {
      	  case CORR_RAW   : bias = 0x7fff;break;
           case CORR_BIAS  : bias = div_s(1,nx);break;
           case CORR_UNBIAS : bias = sub(nz,j);
                              bias = div_s(1,bias);
                              break;
         }
      ...
      for(j = nx-1; j > 0; j--)
    	{
         ...
         {
         	case CORR_RAW   : bias = 0x7fff;break;
            case CORR_BIAS  : bias = add(nx,ny);
                              bias = sub(bias,1);
                              bias = div_s(1,bias);break;
            case CORR_UNBIAS: bias = add(nx,ny);
                              bias = sub(bias,1);
                              bias = sub(bias,j);
                              bias = div_s(1,bias);
                              break;

...
typedef struct dfr16_sFirStructPriv
{
	...
	bool     bCanUseModAddr;     /* Can use modulo addressing on history buffer */
	bool     bCanUseDualMAC;     /* Set if FIR can use dual parallel move MAC */
...
typedef struct dfr16_sIirStruct
{
	...
	Frac16 * pNextHistoryAddr;   /* Next memory element to use */
	bool     bCanUseModAddr;     /* Can use modulo addressing on history buffer */
	bool     bCanUseDualMAC;     /* Set if IIR can use dual parallel move MAC */
} dfr16_tIirStruct;

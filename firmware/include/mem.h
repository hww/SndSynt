...
#define __memory_H
	...
	#include "configdefines.h"
		...
		#error INCLUDE_MEMORY must be defined in appconfig.h to initialize the Memory Library
	#endif
#endif
...
EXPORT void        memValidate      (const mem_sPool *);
...
EXPORT void        memPrintAllocatedBlocks (mem_sPool *);
EXPORT size_t      memCleanUp       (mem_sPool *);

EXPORT void        memCalculateStatistics(mem_sPool *);
EXPORT void        memPrintStatistics    (mem_sPool *);

#ifdef MEM_CHECK_CORRUPTION
#define memMalloc(pPool,Size) memMallocWrapper(pPool,Size,__FILE__,__LINE__)
#endif

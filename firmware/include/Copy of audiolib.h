...
******************************************************************************/
#undef FX_ON								// спецэфекты включены
...
#define MAX_CHANNELS 24						// количество МИДИ каналов
#define MIX_BUF_SIZE 640					// буфер миксера семплов 10 мс
#define FADE_BUF_SIZE 128					// буфер fadeout семплов 4 мкс
...
#define FADE_SUB_VAL 16					// быстрота fadeout
/***********************************************************************
 * Link betwin records
 ***********************************************************************/
...
#define AL_SF_FADEOUT   0x0002	// Семпл имеет петлю
...
// Переменные полифонического голоса
    ALLink  node;		 		// связка
	UInt16  priority;			// указатель на виртуальный голос
	...
	UInt16 	rvol;				// громкость правого канала					
	UInt16 	lvol;				// громкость левого канала
    UInt16	rfxmix;				// Уровень FX	
    UInt16	lfxmix;				// Уровень FX	
	UInt16	volf;				// текущая громкость дробная часть
	Int16 	vol;                // текущая громкость ( обязательно следом за volf !!)
    Int16	volgain;			// Масштаб громкости 0..7FFF
	Int32	volinc;				// прирост громкости 
    Int16   voltg;				// громкость к которой стремимся
    UInt16 	panf;               // панорама дробная часть
    Int16 	pan;                // текущая панорама ( обязательно следом за pdnf !)
    Int16 	pantg;              // панорама к которой стремимся
	Int32	paninc;				// прирост громкости 
    Int16	fxmix;				// Уровень FX	 
    UInt16	fadeval;			// Уровень fadeout   
...
{
    ALLink            node;		 // связка
    struct PVoice_s  *pvoice;    // Указатель на полифонический голос
    ALWaveTable 	 *wavetable; // параметры волновой формы
    UInt16 	          priority;  // приоритет семпла
	s16				  state;	 // состояние голоса
} ALVoice;
...
typedef struct
{
	...
	stereo32   *vol_buf;		// Указатель на буфер громкости
    UInt16		fxptrnum;		// количество выходных поинтеров
    UInt16		fxinptr;		// указатель входных данных FX 
    UInt16		fxdelay[FX_OUT_MAX_POINTERS];	// Задержка в семплах
    UInt16		fxvol[FX_OUT_MAX_POINTERS];		// Громкость 
} ALSynth;
